...
public class CuckooHashMap<K, V> extends AbstractMap<K, V> implements
	...
	public V get(Object key) {
		...
		for(int i =0;i<4;i++){
			int hash = hash(hash1, k)/4 * 4;
			Object k2;
			Entry<K, V> e = table[hash+i];
			if (e != null && ((k2 = e.key) == k || k.equals(k2))) {
				return e.value;
			}

			hash = hash(hash2, k)/4 * 4;
			e = table[hash+i];
			if (e != null && ((k2 = e.key) == k || k.equals(k2))) {
				return e.value;
			}
	...
	private boolean insertEntry(Entry<K, V> e) {
		int count = 0;
		Entry<K, V> current = e;
		int index = hash(hash1, current.key)/4 * 4;//桶编号
		while (current != e || count < table.length) {
			Entry<K, V> temp0 = table[index];
			Entry<K, V> temp1 = table[index+1];
			Entry<K, V> temp2 = table[index+2];
			Entry<K, V> temp3 = table[index+3];
			Random random = new Random();
			int ra = random.nextInt(4);//桶内有四个slots
			switch(ra){
			case 0:
				break;
			case 1:
			...
			if (temp0 == null) {
				table[index+ra] = current;
				return true;
			}

			table[index + ra] = current;
			current = temp0;
			if (index == hash(hash1, current.key)/4 *4) {
				index = hash(hash2, current.key)/4 * 4;
			} else {
				index = hash(hash1, current.key)/4 * 4;
			}
